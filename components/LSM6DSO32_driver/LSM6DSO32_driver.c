...
esp_err_t SPI_read(uint8_t* dataArr, uint8_t LSM6_addr, uint8_t len) {
...
esp_err_t LSM6_begin(uint8_t EN_PIN) {
    ...
    SPI_write(&settings3c, CTRL3_C, 1);
...
esp_err_t LSM6_aclSettings(uint8_t settings, uint8_t resolution, uint8_t ODR, uint8_t LPF, uint8_t EN_PIN) {
    ...
    return SPI_write(&defaultSettingsAcl, CTRL1_XL, 1);
	
}

esp_err_t LSM6_getRawData(uint8_t* rawData) {

   return  SPI_read(rawData, OUTX_L_G, 12);
}
...
esp_err_t LSM6_soft_reset() {
esp_err_t status = ESP_OK;
    settings3c |= 0b10000000;
    status = SPI_write(&settings3c, CTRL3_C, 1);
    settings3c &= 0b01111111;
	return status;
}
