...
typedef struct{
	...
	struct{
		float altitude_press;	/*!< Altitude from pressure sensor. */

		float posX;		/*!< Position on the X-axis. */
		float posY;		/*!< Position on the Y-axis. */
		float posZ;		/*!< Position on the Z-axis. */

		float veloX;	/*!< Velocity on the X-axis. */
		float veloY;	/*!< Velocity on the Y-axis. */
		float veloZ;	/*!< Velocity on the Z-axis. */

		float q1, q2, q3, q4;	/*!< Quaternions.  */
	...
	} ign;	/*!< Ignition information. */


	uint32_t vbat_mV;	/*!< Battery voltage (in millivolts). */

	struct{
		float servo_1;		/*!< Position of servo 1 (in ms). */
		float servo_2;		/*!< Position of servo 2 (in ms). */
		float servo_3;		/*!< Position of servo 3 (in ms). */
		float servo_4;		/*!< Position of servo 4 (in ms). */
		uint8_t servo_en;	/*!< Servo enable flag. */
	} servo;	/*!< Servo positions and enable status. */
} DataPackage_t;
...
typedef struct __attribute__((__packed__)){
	...
	int16_t gyroX_10;	/*!< Angular velocity on the X axis (in tenths of degrees per second [deg/s*10]). */
