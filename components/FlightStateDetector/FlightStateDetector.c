...
static void FlightState_APOGEE				(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
static void FlightState_FREEFALL			(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
static void FlightState_DRAGCHUTE_DEPLOY	(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
static void FlightState_DRAGCHUTE_FALL		(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
static void FlightState_DRAGCHUTE_ERROR		(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
static void FlightState_MAINSHUTE_DEPLOY	(uint64_t time_ms, FlightState_t * currentState, AHRS_t * ahrs);
...
armingstatus_t armstatus_d = DISARMED;
...
esp_err_t FSD_detect(uint64_t time_ms){
	...
	case FLIGHTSTATE_APOGEE:
		FlightState_APOGEE(time_ms, currentState, ahrs);
		break;
	...
	case FLIGHTSTATE_DRAGCHUTE_DEPLOY:
		FlightState_DRAGCHUTE_DEPLOY(time_ms, currentState, ahrs);
		break;
	...
	case FLIGHTSTATE_DRAGCHUTE_ERROR:
		FlightState_DRAGCHUTE_ERROR(time_ms, currentState, ahrs);
		break;

	case FLIGHTSTATE_MAINSHUTE_DEPLOY:
		FlightState_MAINSHUTE_DEPLOY(time_ms, currentState, ahrs);
		break;
